...
function createAPIServer() {
	...
	if (isDev) {
		...
		const frontendEnable =
			process.env.AGENTFABRIC_FRONTEND_ENABLED || AGENTFABRIC_FRONTEND_ENABLED; // default true
	...
	return {
		start: async () => {
			try {
				await app.listen({
					port: AGENTFABRIC_API_SERVER_PORT,
					host: "0.0.0.0",
				});
				app.log.info(
					`API server running on http://localhost:${AGENTFABRIC_API_SERVER_PORT}`,
				);
			} catch (err: unknown) {
				if ((err as { code?: string })?.code === "EADDRINUSE") {
					app.log.error(
						`Port ${AGENTFABRIC_API_SERVER_PORT} is already in use.`,
					);
				} else {
					app.log.error(err);
				}
